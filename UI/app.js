...
const welcomeText =
  "Xin chào, tôi là trợ lý Bệnh viện Hạnh Phúc. Bạn có thể hỏi về gói khám, bác sĩ, thuốc, lịch hẹn hoặc các bước cần chuẩn bị trước khi đến viện.";
...
newChatButtonEl.addEventListener("click", () => {
  resetConversation();
  window.dispatchEvent(new CustomEvent("medical-chat:new-chat"));
});
...
function appendAssistantMessage(text, options = {}) {
  const wrapper = createMessageShell("assistant", "Trợ lý bệnh viện");
...
function syncComposerState() {
  sendButtonEl.disabled = state.busy;
  inputEl.disabled = state.busy;
}

window.__medicalChatApp = {
  ...
  setConversationId(value) {
    state.conversationId = value || null;
  },
  scrollToBottom,
};

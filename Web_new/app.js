...
const state = {
  ...
  doctorMedicineTabs: {
    tabA: [
      { id: "A01", customer: "Nguyễn Văn An", done: false },
      { id: "A02", customer: "Trần Thị Bình", done: false },
      { id: "A03", customer: "Lê Minh Châu", done: false },
      { id: "A04", customer: "Phạm Quốc Dũng", done: false },
      { id: "A05", customer: "Hoàng Thị Hà", done: false },
      { id: "A06", customer: "Võ Gia Huy", done: false },
      { id: "A07", customer: "Đặng Kim Lan", done: false },
      { id: "A08", customer: "Bùi Thanh Sơn", done: false },
    ],
    tabB: [
      { id: "B01", customer: "Ngô Mai Anh", done: false },
      { id: "B02", customer: "Đỗ Đức Bảo", done: false },
      { id: "B03", customer: "Huỳnh Nhật Linh", done: false },
      { id: "B04", customer: "Mai Hồng Ngọc", done: false },
      { id: "B05", customer: "Phan Tuấn Kiệt", done: false },
      { id: "B06", customer: "Tạ Quỳnh Như", done: false },
      { id: "B07", customer: "Cao Minh Phúc", done: false },
      { id: "B08", customer: "Lâm Thu Trang", done: false },
    ],
  },
};
...
const mapButtonEl = document.querySelector("#mapButton");
...
const doctorSettingsFormEl = document.querySelector("#doctorSettingsForm");
const apiProviderSelectEl = document.querySelector("#apiProviderSelect");
const apiModelSelectEl = document.querySelector("#apiModelSelect");
const apiBaseUrlInputEl = document.querySelector("#apiBaseUrlInput");
const apiKeyInputEl = document.querySelector("#apiKeyInput");
const volumeRangeEl = document.querySelector("#volumeRange");
...
const doorToggleButtonEl = document.querySelector("#doorToggleButton");
const doctorDoorStatusEl = document.querySelector("#doctorDoorStatus");
const rotateTrayButtonEl = document.querySelector("#rotateTrayButton");
const doctorTrayStatusEl = document.querySelector("#doctorTrayStatus");
const doctorArduinoStatusEl = document.querySelector("#doctorArduinoStatus");
const doctorMedicineRowsEl = document.querySelector("#doctorMedicineRows");
...
function updateClock() {
  ...
  dateTextEl.textContent = new Intl.DateTimeFormat("vi-VN", {
    day: "2-digit",
    month: "2-digit",
    year: "numeric",
  }).format(now);
...
function initDoctorPanel() {
  state.doctorSettings = loadDoctorSettings();
  applyDoctorSettingsToForm();
  renderDoctorStatus();
  ...
  volumeRangeEl.addEventListener("input", updateVolumeLabel);
  apiProviderSelectEl.addEventListener("change", () => {
    populateApiModelOptions(apiProviderSelectEl.value);
  });

  excelUploadButtonEl.addEventListener("click", () => excelUploadInputEl.click());
  excelUploadInputEl.addEventListener("change", handleExcelUpload);
  doorToggleButtonEl.addEventListener("click", toggleDoctorDoor);
  rotateTrayButtonEl.addEventListener("click", rotateDoctorTray);

  doctorTabButtons.forEach((button) => {
    button.addEventListener("click", () => setDoctorTab(button.dataset.doctorTab || "tabA"));
  });
}
...
function loadDoctorSettings() {
  const defaults = getDefaultDoctorSettings();
  try {
    const saved = window.localStorage.getItem(DOCTOR_SETTINGS_STORAGE_KEY);
    return saved ? { ...defaults, ...JSON.parse(saved) } : defaults;
  } catch (error) {
    console.warn("Cannot load doctor settings", error);
    return defaults;
  }
}
...
function saveDoctorSettings(settings) {
  try {
    window.localStorage.setItem(DOCTOR_SETTINGS_STORAGE_KEY, JSON.stringify(settings));
  } catch (error) {
    console.warn("Cannot save doctor settings", error);
  }
}

function applyDoctorSettingsToForm() {
  const settings = state.doctorSettings || getDefaultDoctorSettings();
  const provider = API_MODEL_OPTIONS[settings.apiProvider] ? settings.apiProvider : "chatgpt";
  apiProviderSelectEl.value = provider;
  populateApiModelOptions(provider, settings.apiModel);
  apiBaseUrlInputEl.value = settings.apiBaseUrl || "";
  apiKeyInputEl.value = settings.apiKey || "";
  volumeRangeEl.value = String(settings.volume ?? 70);
...
function collectDoctorSettingsFromForm() {
  return {
    ...
    volume: Number(volumeRangeEl.value || 70),
    arduinoPort: arduinoPortInputEl.value.trim(),
...
function updateVolumeLabel() {
  volumeValueEl.textContent = `${volumeRangeEl.value || 0}%`;
}

function populateApiModelOptions(provider, selectedModel = "") {
  const options = API_MODEL_OPTIONS[provider] || API_MODEL_OPTIONS.chatgpt;
  apiModelSelectEl.innerHTML = options
    .map((option) => `<option value="${option.value}">${option.label}</option>`)
    .join("");
  const modelExists = options.some((option) => option.value === selectedModel);
  apiModelSelectEl.value = modelExists ? selectedModel : options[0].value;
}

function openDoctorSettings() {
  applyDoctorSettingsToForm();
  settingsPanelEl.hidden = false;
  apiProviderSelectEl.focus();
}
...
function renderDoctorStatus() {
  doctorDoorStatusEl.textContent = state.doctorDoorOpen ? "Đang mở" : "Đang đóng";
  doorToggleButtonEl.classList.toggle("is-active", state.doctorDoorOpen);
  doorToggleButtonEl.querySelector("strong").textContent = state.doctorDoorOpen ? "Đóng cửa" : "Mở cửa";

  doctorTrayStatusEl.textContent = state.doctorTrayBusy ? "Đang xoay" : "Sẵn sàng";
  rotateTrayButtonEl.classList.toggle("is-busy", state.doctorTrayBusy);
  rotateTrayButtonEl.disabled = state.doctorTrayBusy;
  rotateTrayButtonEl.querySelector("strong").textContent = state.doctorTrayBusy ? "Đang xoay..." : "Xoay khay";

  const port = state.doctorSettings?.arduinoPort?.trim();
  doctorArduinoStatusEl.textContent = port ? `Arduino: ${port}` : "Arduino: chờ kết nối";
}

function handleExcelUpload() {
  const file = excelUploadInputEl.files?.[0];
  excelUploadStatusEl.textContent = file ? `Đã chọn: ${file.name}` : "Chưa chọn file tra cứu";
}

function toggleDoctorDoor() {
  state.doctorDoorOpen = !state.doctorDoorOpen;
  renderDoctorStatus();
}

function rotateDoctorTray() {
  if (state.doctorTrayBusy) {
    return;
  }
  state.doctorTrayBusy = true;
  renderDoctorStatus();
  window.setTimeout(() => {
    state.doctorTrayBusy = false;
    renderDoctorStatus();
  }, 900);
}

function setDoctorTab(tabName) {
  if (!state.doctorMedicineTabs[tabName]) {
    return;
  }
  state.selectedDoctorTab = tabName;
  renderDoctorMedicineList();
}

function renderDoctorMedicineList() {
  doctorTabButtons.forEach((button) => {
    const isActive = button.dataset.doctorTab === state.selectedDoctorTab;
    button.classList.toggle("is-active", isActive);
    button.setAttribute("aria-selected", String(isActive));
  });

  const rows = state.doctorMedicineTabs[state.selectedDoctorTab] || [];
  doctorMedicineRowsEl.innerHTML = rows
    .map(
      (item, index) => `
        <label class="doctor-medicine-row" role="listitem">
          <span class="order-index">${String(index + 1).padStart(2, "0")}</span>
          <span class="customer-name">${item.customer}</span>
          <input class="setup-check" type="checkbox" data-medicine-id="${item.id}" ${item.done ? "checked" : ""} />
        </label>
      `,
    )
    .join("");

  doctorMedicineRowsEl.querySelectorAll("[data-medicine-id]").forEach((checkbox) => {
    checkbox.addEventListener("change", () => {
      const item = rows.find((entry) => entry.id === checkbox.dataset.medicineId);
      if (item) {
...
async function startQrScanner() {
  ...
  try {
    ...
    if (!("BarcodeDetector" in window)) {
      showQrFallback("Trình duyệt chưa hỗ trợ nhận diện QR.");
      return;
    }
